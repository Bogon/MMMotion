...
- (id)initWithView:(UIView *)view {
    ...
    /// in main thread update UI
    return [self initWithFrame:CGRectMake(0, 64, view.bounds.size.width, view.bounds.size.height-64*2)];
}
...
- (void)dealloc {
    ...
    [_detailsLabelTextColor release];
    [graceTimer release];
...
#pragma mark - 修改HUB view 的位置
    UIView *parent = self.superview;
    if (parent) {
        self.frame = CGRectMake(0, 0, parent.bounds.size.width, parent.bounds.size.height-0);
        //		self.frame = CGRectMake(0, 64, parent.bounds.size.width, parent.bounds.size.height-64);
    }
    ...
    CGRect labelF;
#pragma mark - 文字与图标之间的距离
    CGFloat spaceH = 4;
    labelF.origin.y = yPos + spaceH;
