...
public extension UIScreen {
    ...
    class var ol_orientationSize: CGSize {
        guard UIApplication.ol_sharedApplication() != nil else {
            return CGSize.zero
        }
    ...
    class var ol_screenHeightWithoutNavBar: CGFloat {
        guard let app = UIApplication.ol_sharedApplication() else {
            return 0
        }
        let isLand: Bool = UIInterfaceOrientation.landscapeLeft.isLandscape
        ...
        if #available(iOS 11.0, *) {
            if let topInset = app.keyWindow?.safeAreaInsets.top {
                topHeight += topInset
            }
        }
        
        if isLand {
            return UIScreen.main.bounds.size.height - topHeight
        } else {
            return UIScreen.main.bounds.size.width - topHeight
        }
    }
}
